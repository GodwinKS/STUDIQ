...
export function CommonTool() {
  ...
  const handleVoiceInput = async (base64: string, mimeType: string) => {
    setIsTranscribing(true);
    try {
      const text = await transcribeAudioToText(base64, mimeType, language);
      if (text) {
      ...
      <div className="space-y-8">
        {!selectedImage ? (
          <div
            ...
            className="group h-[320px] rounded-[40px] border-2 border-dashed border-[#141414]/10 hover:border-[#141414]/30 hover:bg-white transition-all cursor-pointer flex flex-col items-center justify-center"
          >
            <div className="p-6 bg-white rounded-full shadow-lg group-hover:scale-110 transition-transform">
              <Camera size={32} />
            </div>
            <div className="mt-6 text-center">
              <p className="text-lg font-bold">Capture or Select Image</p>
              <p className="text-xs opacity-40 mt-1 uppercase tracking-widest font-mono">JPG, PNG, WEBP Supported</p>
            </div>
