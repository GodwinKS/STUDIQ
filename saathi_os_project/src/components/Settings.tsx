  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* local Mode Card */}
        <div className={`p-8 rounded-[40px] border-2 transition-all relative overflow-hidden ${isLocal ? 'bg-[#141414] text-white border-[#141414]' : 'bg-white border-[#141414]/5'}`}>
          {isLocal && <div className="absolute top-0 right-0 w-32 h-32 bg-green-500/10 blur-[60px] -mr-16 -mt-16" />}
          
          <div className="flex justify-between items-start mb-6">
            <div className={`p-4 rounded-2xl ${isLocal ? 'bg-white/10' : 'bg-[#F5F5F0]'}`}>
              <Cpu className={isLocal ? 'text-white' : 'text-[#141414]'} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase tracking-widest opacity-60">Privacy: Ultra</span>
              <div className={`w-2.5 h-2.5 rounded-full ${isLocal ? 'bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.5)]' : 'bg-gray-300'}`} />
            </div>
          </div>
          
          <h3 className="text-2xl font-bold mb-3">Offline Core</h3>
          <p className={`text-sm mb-8 leading-relaxed ${isLocal ? 'text-white/60' : 'text-[#141414]/60'}`}>
            Absolute privacy. Runs on your CPU. No internet required for vision or language tasks.
          </p>
          ...
          <button 
            ...
            className={`w-full py-4 rounded-2xl font-bold text-sm transition-all ${
              isLocal 
                ? 'bg-white text-[#141414] shadow-xl' 
                : 'bg-[#F5F5F0] text-[#141414] hover:bg-[#EBEBE5]'
            }`}
          >
            {isLocal ? 'Currently Active' : 'Switch to Local Core'}
          </button>
        </div>

        {/* Cloud Mode Card */}
        <div className={`p-8 rounded-[40px] border-2 transition-all relative overflow-hidden ${!isLocal ? 'bg-[#141414] text-white border-[#141414]' : 'bg-white border-[#141414]/5'}`}>
          {!isLocal && <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 blur-[60px] -mr-16 -mt-16" />}

          <div className="flex justify-between items-start mb-6">
            <div className={`p-4 rounded-2xl ${!isLocal ? 'bg-white/10' : 'bg-[#F5F5F0]'}`}>
              <Cloud className={!isLocal ? 'text-white' : 'text-[#141414]'} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase tracking-widest opacity-60">Speed: Turbo</span>
              <div className={`w-2.5 h-2.5 rounded-full ${!isLocal ? 'bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.5)]' : 'bg-gray-300'}`} />
            </div>
          </div>
          
          <h3 className="text-2xl font-bold mb-3">Cloud Hybrid</h3>
          <p className={`text-sm mb-8 leading-relaxed ${!isLocal ? 'text-white/60' : 'text-[#141414]/60'}`}>
            Blazing fast. Uses Gemini 2.0. Perfect for long lectures and complex logic.
          </p>
          ...
          >
            {!isLocal ? 'Currently Active' : 'Switch to Cloud'}
          </button>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-2 py-2">
                  {[
                    { id: 'gemma4:e4b', label: 'gemma4:e4b — Recommended (4B)', color: 'bg-green-100 text-green-700' },
                    { id: 'gemma4:e2b', label: 'gemma4:e2b — Lite (2B, fast)', color: 'bg-blue-100 text-blue-700' },
                    { id: 'gemma4:9b', label: 'gemma4:9b — Pro (9B, 16GB+)', color: 'bg-purple-100 text-purple-700' },
                  ].map(m => (
                    <button 
                      key={m.id}
